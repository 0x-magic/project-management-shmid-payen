...
export default function InterviewPage() {
	...
	const columns: ColumnsType<InterviewInfo> = [
		...
		},
		{
			...
			dataIndex: "term",
			width: 100,
			render: (_, record) => <div>{record.term}</div>,
		},
		{
			title: "STATUS",
			dataIndex: "status",
		...
		},
		{
			...
			dataIndex: "source",
			width: 100,
			render: (_, record) => <div>{record.source}</div>,
		},
		{
			title: "POSITION",
			dataIndex: "position",
		...
		{
			title: "1 THROUGH 5 SYSTEM",
			dataIndex: "mark",
			width: 100,
			render: (_, record) => <div>{record.mark}</div>,
		},
		{
			title: "Meeting Link",
